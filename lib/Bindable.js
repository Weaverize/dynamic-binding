...
var Binder = class {
			...
			set(obj, prop, value, parent) {
				if (obj[prop] != value) {
					...
					if (value && typeof value == "object") {
						returnValue = obj[prop] = this.deepBind(obj, prop);
					}
	...
	bindProperty(path, destObj, destProp) {
		var parent = resolveParent(this.element.__data, path);
	...
	/**
	 * Crawls recursively within a tree of object properties and replace the object by proxies
	 * Ignores non object property
	 * FIXME: /!\ Doesn't handle cycling in object /!\
	 * @param {*} obj object to replace by a proxy
	 * @param {String} property name of the property
	 * @returns the proxy replacement to the object
	 */
	deepBind(obj, property) {
		//FIXME: obj[property] is never left-hand could be removed altogether
		//deepBind would only need one parameter then
		if (obj[property] && typeof obj[property] == "object") {
			for (var subprop in obj[property]) {
				...
				if (obj[property][subprop] && typeof obj[property][subprop] == "object") {
					obj[property][subprop] = this.deepBind(obj[property], subprop);
				}
			}
			var proxy = new Proxy(obj[property], this.handler);
			this.register(proxy);
			return proxy;
		}
	}
